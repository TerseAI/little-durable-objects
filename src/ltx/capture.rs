//! Turning committed SQLite WAL frames into durable LTX segments.
//!
//! ```text
//! SQLite WAL ──▶ committed frames ──▶ LTX segment ──▶ <db>-ltx/<min>-<max>.ltx
//! ```
//!
//! Capture is deliberately synchronous and pull-based: nothing happens until
//! [`SqliteLtxCapture::sync`] is called, and every segment it returns is already durable on
//! local disk. LTX encoding itself is delegated to `litetx`, superfly's own
//! implementation of the format.
//!
//! # Durability
//!
//! There is exactly **one durable artifact per newly captured transaction** — the
//! segment file — and it is published atomically. A restored database may instead
//! begin at a compact capture base written as part of the same staged directory swap:
//!
//! * The next TXID is derived from the capture base plus the contiguous tail of segment
//!   files on disk. The base and database are installed together, so they cannot drift.
//! * The WAL position is re-derived at attach by replaying the frames the chain already
//!   accounts for, so it cannot drift either.
//! * In-memory state — cursor, checksums, TXID — advances only *after* the segment it
//!   describes is durable, so a failed encode or write offers the transaction again
//!   instead of skipping it.
//!
//! # Call `sync` before closing the database
//!
//! Closing the last SQLite connection checkpoints the WAL into the database file. Any
//! transaction capture has not read by then is folded away, and cannot be reconstructed
//! as a segment afterwards — LTX permits a full snapshot only at TXID 1, which is
//! already spoken for. Attach detects this and fails with
//! [`LtxError::UncapturedWrites`](crate::ltx::error::LtxError::UncapturedWrites) rather than
//! emitting a segment no replica could apply. A crash is *recoverable*, because the WAL
//! survives it; a clean close without a final `sync` is not.
//!
//! Lifting that restriction means generations — restarting the chain at TXID 1 in a new
//! directory — which is deliberately left for later.
